...
export function AuthGateway({ serverMode, instanceName, initialUsername, displayName, error, loading, setupRequired = false, credentialInvalid = false, authMethod = '', onSubmit, onPersonalSetup, onPersonalUnlock, onPersonalCredential }: { serverMode: 'global' | 'selfhosted'; instanceName: string; initialUsername: string; displayName?: string; error: string; loading: boolean; setupRequired?: boolean; credentialInvalid?: boolean; authMethod?: PersonalAuthMethod | ''; onSubmit: (username: string, password: string) => Promise<void>; onPersonalSetup: (username: string, password: string, method: PersonalAuthMethod, secret: string) => Promise<void>; onPersonalUnlock: (secret: string) => Promise<void>; onPersonalCredential: (password: string) => Promise<void> }) {
  ...
  if (serverMode === 'selfhosted') return <PersonalAccess setupRequired={setupRequired} credentialInvalid={credentialInvalid} authMethod={authMethod} username={initialUsername} displayName={displayName} instanceName={instanceName} error={error} loading={loading} onSetup={onPersonalSetup} onUnlock={onPersonalUnlock} onCredential={onPersonalCredential} />;
