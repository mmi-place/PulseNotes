...
export function EvaluationInsight({ evaluation, demo, changeState = 'seen', onSeen, studentName = '', formation = '', semesterLabel = '' }: { evaluation: Evaluation; demo: boolean; changeState?: EvaluationChangeState; onSeen?: (id: string) => void; studentName?: string; formation?: string; semesterLabel?: string }) {
  ...
  return <div ref={target} className={`evaluation-insight available ${unread ? `unread ${changeState}` : ''}`} onClick={() => unread && onSeen?.(evaluation.id)}>
    ...
    <dialog ref={dialog} className="distribution-dialog" onClick={event => { if (isDialogBackdropClick(event)) { event.preventDefault(); event.stopPropagation(); dialog.current?.close(); } }} onClose={() => unread && onSeen?.(evaluation.id)}>
      <div className="distribution-dialog-actions">{!demo && <button type="button" className="share-trigger" onClick={() => void openSharing()}>Partager</button>}<button className="dialog-close" onClick={() => dialog.current?.close()}>Fermer</button></div>
