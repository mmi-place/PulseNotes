...
RELEASE_URL="${PULSENOTES_RELEASE_URL:-https://github.com/mmi-place/PulseNotes/releases/latest/download/pulsenotes-global.zip}"
TMP_DIR="$(mktemp -d)"
ROLLBACK=0
...
for command in curl unzip php sha256sum; do
  command -v "$command" >/dev/null 2>&1 || { echo "$command est requis." >&2; exit 1; }
done
test -f "$APP_DIR/api/config.php" || { echo 'Configuration globale introuvable.' >&2; exit 1; }
...
ROLLBACK=0
echo 'PulseNotes global a été mis à jour. La configuration et la base distante sont conservées.'
